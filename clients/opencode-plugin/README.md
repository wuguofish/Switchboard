...
| 變數 | 預設 | 用途 |
|---|---|---|
| `SWITCHBOARD_URL` | `http://127.0.0.1:9876` | Switchboard daemon 位址 |
| `SWITCHBOARD_PEER_ALIAS` | `opencode` | headless server 註冊的總機門牌 |
| `SWITCHBOARD_PEER_PREFIX` | （空） | 分機 alias 前綴，server 與 TUI plugin 共用；留空則門牌就是標題本身 |
...
- Headless server 是唯一註冊總機門牌（`SWITCHBOARD_PEER_ALIAS`，預設 `opencode`）的行程。
...
- **總機**：只有 headless server plugin 負責，常駐接收寄給總機門牌（`SWITCHBOARD_PEER_ALIAS`）的信。
...
- **分機命名**：alias 就是 session 標題的 slug，跟其他 Switchboard client 自己取名的門牌長得一樣（例如標題 `ios-4305` → 門牌 `ios-4305`）。只有標題真的撞名時才依序補 session ID 末 8 碼、完整 session ID。設了 `SWITCHBOARD_PEER_PREFIX` 會原樣接在前面（`<前綴>-<標題slug>`）。標題變更會由目前 owner renew alias。
...
2. 直接執行 `opencode`（不是 attach），進入一個具名 session；確認 Switchboard 出現與標題 slug 同名的分機且在線。

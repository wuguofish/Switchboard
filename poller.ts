...
export async function main() {
  ...
  const homeDir = homedir().replaceAll('\\', '/')
  const config = loadConfigFromHookStdin(input, {
    dbPath: process.env.SWITCHBOARD_DB ?? `${homeDir}/.claude/switchboard.db`,
    stateFileDir:
      process.env.SWITCHBOARD_POLLER_STATE_DIR ?? `${homeDir}/.claude`,
  })
  if (!config) {
    process.exit(0)
  }

...
- `<state_dir>/switchboard-poller-<cc_session_id>.state`（`state_dir` 預設 `<homedir>/.claude`，由 `SWITCHBOARD_POLLER_STATE_DIR` 覆寫）— 舊版 `bun poller.ts` 會寫，留作 fallback 自己的 parent-pid check 用。新版 `poller-shim.ps1` 不寫 state file。`canAutoWake`（決定 `delivered_notification`/`notified_count`）**只看** daemon 的 in-memory `waiters.isPolling`，不再讀 state file——state file 的 pid 可能被作業系統 reuse 造成假陽性

### 舊版 `bun poller.ts`（仍可用作 fallback）

舊的 Stop hook command `bun <switchboard-repo>/poller.ts` 仍然能 work — 會寫 state file，parent-pid alive check 也已就位。但每個 poller 子進程 ~150 MB，並行多個 session 時記憶體壓力大。預設改用 shim，舊版只推薦在 PowerShell 不可用的環境使用。

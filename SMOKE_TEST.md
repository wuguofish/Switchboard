...
2. The test workspace's `.mcp.json` (or user-scope MCP config) has the `switchboard` entry pointing at `http://127.0.0.1:9876/mcp`
3. The test workspace's `.claude/settings.local.json` has the SessionStart hook (see `client-hooks.example.json`)

## Two-session round trip

Open **two** Claude Code sessions (two terminal windows) in your test workspace.
...
3. Check `<state_dir>` (default `<homedir>/.claude`) — there should be TWO state files: `switchboard-poller-<cc-a>.state` and `switchboard-poller-<cc-b>.state`, each with a different pid

...
# State dir for log + lock files. Mirrors poller.ts SWITCHBOARD_POLLER_STATE_DIR
# default ($HOME/.claude). Created on first run if missing so log/lock writes
# don't silently fail under try/catch.
$stateDir = if ($env:SWITCHBOARD_POLLER_STATE_DIR) { $env:SWITCHBOARD_POLLER_STATE_DIR } else { ($env:USERPROFILE -replace '\\', '/') + '/.claude' }
try { New-Item -ItemType Directory -Path $stateDir -Force -ErrorAction SilentlyContinue | Out-Null } catch {}
...
$logPath = "$stateDir/switchboard-shim.log"
...
$lockPath = "$stateDir/switchboard-shim-$ccSessionId.lock"

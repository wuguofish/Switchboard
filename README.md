...
1. Add the MCP server to your workspace's `.mcp.json`. For `socket` and `monitor`, connect to the daemon directly:
    ...
    For `channel`, point it at the shim (see [`clients/cc-channel/`](clients/cc-channel/README.md)):
...
- `GET /monitor?cc_session_id=<uuid>` — persistent chunked text stream for Claude Code's `Monitor` tool. One line per event:
  ...
  - `heartbeat <iso-ts>` — emitted every 4 hr of silence as a visible time tick, e.g. `heartbeat 2026-04-24(五)T13:38:25+08:00`; the `(X)` after the date is the Taipei weekday, so subscribers read it rather than deriving it and getting the UTC/Taipei day boundary wrong (a silent space byte goes out every 240s to keep Bun's `idleTimeout` from cutting the stream)
